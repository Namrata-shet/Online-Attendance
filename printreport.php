<title>Online Attendance - computer dept</title>
<style type="text/css">
<!--
.style1 {
...
<table width="800" border="1" align="center">
      ...
      <tr>
        ...
        <form action="" method="post">
          <table width="606" border="2" align="center" bordercolor="#9966FF" bgcolor="#C7B6B1">
            ...
            <tr>
              <td width="308" bgcolor="#9999CC"><div align="center"><strong><span class="style2">Enter the enrollment no</span></strong></div></td>
    ...
    if(isset($_POST["btnsubmit"]))
    {
      include "z_db.php";
      extract($_POST);
      $query = "select * from member where enrollment_no = ".$eno." limit 1";

      $result = mysqli_query($con,$query)or die("select error error 51");
      while($rec = mysqli_fetch_array($result))
      {
        ...
        <tr>
          ...
          $result1 = mysqli_query($con,$query1)or die("select error error 59");
        ...
        echo '</tr>
        <tr>
          <td width="222"><span class="style6">'.$rec["enrollment_no"].'</span></td>
          <td width="222"><span class="style6">'.$rec["name"].'</span></td>';
          $query1 = "select *from `attendance` where `member_id` = ".$rec["member_id"]." order by date";
          $result1 = mysqli_query($con,$query1)or die("select error error 69");
          while($rec1 = mysqli_fetch_array($result1))
          {
              echo '<td>';
    ...
    else
    {
      ...
      $result = mysqli_query($con,$query)or die("select error error 92");
      while($rec = mysqli_fetch_array($result))
      {
        ...
        <tr>
          ...
          <td width="160" bgcolor="#9999CC"><span class="style2">Name</span></td>';
          $query1 = "select * from `attendance` where `member_id` = ".$rec["member_id"]." order by date";
          $result1 = mysqli_query($con,$query1)or die("select error error 100");
          while($rec1 = mysqli_fetch_array($result1))
          {
              echo '<td bgcolor="#9999CC" class=style2>'.$rec1["date"].'</td>';
          }
        ...
        <tr>
          ...
          <td width="222"><span class="style6">'.$rec["name"].'</span></td>';
          $query1 = "select *from `attendance` where `member_id` = ".$rec["member_id"]." order by date";
          $result1 = mysqli_query($con,$query1)or die("select error error 110");
          while($rec1 = mysqli_fetch_array($result1))
          {
              echo '<td>';
            if($rec1["attendance"]==0)
